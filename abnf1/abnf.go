...
var naiveABNF = p.MakeRules(map[string]p.Consumer{
	"rulelist":      p.Plus(p.Alt(p.Ref("rule"), p.B(p.Cat(p.Star(p.Ref("c-wsp")), p.Ref("c-nl"))))),
	"rule":          p.Cat(p.Ref("rulename"), p.Ref("defined-as"), p.Ref("elements"), p.B(p.Ref("c-nl"))),
	"rulename":      p.Cat(p.ALPHA, p.Star(p.Alt(p.ALPHA, p.DIGIT, p.Lit('-')))),
	"defined-as":    p.Cat(p.B(p.Star(p.Ref("c-wsp"))), p.Alt(p.Lit('='), p.Cat(p.Lit('='), p.Lit('/'))), p.B(p.Star(p.Ref("c-wsp")))),
	"elements":      p.Cat(p.Ref("alternation"), p.B(p.Star(p.Ref("c-wsp")))),
	"c-wsp":         p.Alt(p.WSP, p.Cat(p.Ref("c-nl"), p.WSP)),
	"c-nl":          p.Alt(p.Ref("comment"), p.B(p.CRLF), p.B(p.LF)),
	"comment":       p.Cat(p.Lit(';'), p.Star(p.Alt(p.WSP, p.VCHAR)), p.B(p.Alt(p.CRLF, p.LF))),
	"alternation":   p.Cat(p.Ref("concatenation"), p.Star(p.Cat(p.B(p.Star(p.Ref("c-wsp"))), p.B(p.Lit('/')), p.B(p.Star(p.Ref("c-wsp"))), p.Ref("concatenation")))),
	"concatenation": p.Cat(p.Ref("repetition"), p.Star(p.Cat(p.B(p.Plus(p.Ref("c-wsp"))), p.Ref("repetition")))),
	...
	"group":         p.Cat(p.Lit('('), p.B(p.Star(p.Ref("c-wsp"))), p.Ref("alternation"), p.B(p.Star(p.Ref("c-wsp"))), p.Lit(')')),
	"option":        p.Cat(p.Lit('['), p.B(p.Star(p.Ref("c-wsp"))), p.Ref("alternation"), p.B(p.Star(p.Ref("c-wsp"))), p.Lit(']')),
